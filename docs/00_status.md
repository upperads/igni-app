...
- **Execução — ondas**:
  ...
  - Wave 3d (UI) — **em andamento.** ✅ *Fundação visual*: design system do 03b (tokens grafite/sinal/âmbar, fontes placa+instrumento), componentes-assinatura (RiskRail, OsCard com espinha de status, StatusPill, KpiStat, Button, AppShell desktop/mobile) e Home-vitrine do painel do gestor. Verificado por screenshot (desktop + mobile). Commit `edfef0f`. *Faltam as telas:* onboarding/login/2FA + sessão `@supabase/ssr` + recuperação + força de senha + read-only por papel + **US-17 "Primeiros passos"** — card na Home + rota `/primeiros-passos`, copy anti-IA, visual impecável; conteúdo em `docs/conteudo/primeiros-passos.md`), Next 16 — pendente.
