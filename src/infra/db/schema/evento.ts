...
export const evento = pgTable(
  ...
  (t) => [
    // Perf (evita full scan da tabela que mais cresce): o relatório/histórico filtra por janela de
    // tempo do tenant (`historicoResponsabilidade`/`relatorioDeGestao` usam `em >= desde`).
    index("evento_tenant_em_idx").on(t.tenantId, t.em),
    // Perf: a linha do tempo do detalhe da OS filtra por `os_id` + ordena por `em`.
    index("evento_os_idx").on(t.osId),
  ],
);
